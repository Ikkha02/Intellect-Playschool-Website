   ...
   if(!empty($_FILES['Aimage']['name'])) {
    ...
    if ($error === 0) {
      if ($img_size > 100000000) {
        echo "<script>
        window.location.href='announcementmodifadmin.php?id=".$_POST['Aid']."';
        alert('Sorry, your inserted file is too large');
        </script>"; 
      }else {
          ...
          echo "<script>
           window.location.href='announcementmodifyadmin.php?id=".$_POST['Aid']."';
